...
- We use ``Ruff`` for code formatting. ``Ruff`` is a lightning-fast linter and formatter
  that enforces consistent and clean Python code.

- Our test framework is ``pytest``. ``pytest`` offers a powerful, flexible, and user-friendly
  approach to writing and executing tests.

- ``FlexGet``'s Web UI is built with ``React``. ``React`` is a modern JavaScript library for
  creating dynamic, responsive, and interactive user interfaces.
