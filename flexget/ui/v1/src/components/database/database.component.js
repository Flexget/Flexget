...
(function () {
  ...
  angular.module('components.database').component('databaseSidebar', {
  ...
  function databaseController($mdDialog, $sce, $mdToast, databaseService) {
    ...
    function activate() {
      databaseService.getPlugins().then(setPlugins).cached(setPlugins);
    }
    ...
    function cleanup() {
      databaseService.cleanup().then(openSuccess);
    }

    function vacuum() {
      databaseService.vacuum().then(openSuccess);
    }
    ...
    function searchPlugin(query) {
      var results = query
        ? vm.plugins.filter(createFilterFor(query))
        : vm.plugins;
      return results;
    ...
    function resetPlugin() {
      var confirm = $mdDialog
        ...
        .htmlContent(
          $sce.trustAsHtml(
            'Are you sure you want to reset the database for <b>' +
              vm.selectedPlugin +
              '</b>?',
          ),
        )
      ...
      $mdDialog.show(confirm).then(function () {
        ...
        databaseService.resetPlugin(params).then(openSuccess);
      });
    }
  }
})();
