...
(function () {
  ...
  angular.module('blocks.pagination').component('fgPagination', {
  ...
  function paginationController(linkHeaderParser) {
    ...
    function handleChanges(changesObj) {
      if (changesObj.linkHeader) {
        var links = changesObj.linkHeader.currentValue
          ? linkHeaderParser.parse(vm.linkHeader)
          : {};
    ...
    function linkGroupFirst() {
      var rightDebt = vm.last
        ? Math.max(0, +vm.currentPage - (+vm.last.page - 2))
        : 0;
      return Math.max(1, +vm.currentPage - rightDebt - 2);
    }

    function linkGroupLast() {
      var leftDebt = Math.max(0, 1 + 2 - +vm.currentPage);
      return Math.min(
        vm.last ? +vm.last.page : 0,
        +vm.currentPage + leftDebt + 2,
      );
    }
