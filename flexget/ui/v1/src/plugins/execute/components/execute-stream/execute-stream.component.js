...
(function () {
  ...
  angular.module('plugins.execute').component('executeStream', {
    templateUrl:
      'plugins/execute/components/execute-stream/execute-stream.tmpl.html',
    controllerAs: 'vm',
  ...
  function executeStreamController($filter, $log, executeService) {
    ...
    function sortStreamTasks(taskData) {
      ...
      for (var i = 0; i < taskData.length; i++) {
        var emptyTask = $filter('filter')(notOrdered, {
    ...
    function startStream() {
      ...
      function progressNode(progress) {
        var filtered = $filter('filter')(vm.streamTasks, {
          status: '!complete',
        });
