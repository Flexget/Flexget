...
gulp.task('serve-specs', ['build-specs'], function (done) {
  gulp.watch(
    [paths.src + '/**/*.js', paths.spechelpers + '/**/*.js'],
    function () {
      browserSync.reload();
    },
  );
...
gulp.task('build-specs', function () {
  ...
  var specHelpers = gulp.src([paths.spechelpers + '/**/*.js']);

  var specs = gulp.src([paths.src + '/**/*.spec.js']);

  return gulp
    .src(paths.specs + '/specs.html')
    .pipe($.inject(injectScripts))
    .pipe(wiredep(wiredepOptions))
    .pipe(
      $.inject(specHelpers, {
        starttag: '<!-- inject:spechelpers:{{ext}} -->',
      }),
    )
