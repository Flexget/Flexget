...
def make_url(imdb_id):
    ...
    return f'http://www.imdb.com/title/{imdb_id}/'
...
class ImdbSearch(object):
    ...
    def best_match(self, name, year=None, single_match=True):
        ...
        if diff < self.min_diff:
            log.debug('unable to determine correct movie, min_diff too small (`%s` <-?-> `%s`)', movies[0], movies[1])
            for m in movies:
...
class ImdbParser(object):
    ...
    def parse(self, imdb_id):
        ...
        if name_elem:
            ...
            raise PluginError(f'Unable to set imdb_name for {self.imdb_id} from {self.url}')
