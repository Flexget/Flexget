...
class SftpClient:
    def __init__(
        ...
        self.prefix: str = url_prefix
        self._sftp_connection: 'pysftp.Connection' = sftp_connection
        self._handler_builder: HandlerBuilder = handler_builder
    ...
    def list_directories(
        ...
        dir_handler: NodeHandler = self._handler_builder.get_dir_handler(
            get_size, files_only, entry_accumulator
        )
        file_handler: NodeHandler = self._handler_builder.get_file_handler(get_size, entry_accumulator)
        unknown_handler: NodeHandler = self._handler_builder.get_unknown_handler()

        for directory in directories:
            try:
                self._sftp_connection.walktree(
                    directory, file_handler, dir_handler, unknown_handler, recursive
                )
            except OSError as e:
                logger.warning('Failed to open {} ({})', directory, str(e))

        return entry_accumulator
    ...
    def download(self, source: str, to: str, recursive: bool, delete_origin: bool) -> None:
        ...
        if self.is_file(source):
            ...
            try:
                self._sftp_connection.cwd(source_dir)
            ...
            try:
                self._sftp_connection.cwd(base_path)
                self._sftp_connection.walktree(dir_name, handle_file, dir_handler, unknown_handler, recursive)
            except Exception as e:
    ...
    def remove_dir(self, path: str) -> None:
        ...
        """
        if self._sftp_connection.exists(path) and not self._sftp_connection.listdir(path):
            logger.debug('Attempting to delete directory {}', path)
            try:
                self._sftp_connection.rmdir(path)
            except Exception as e:
    ...
    def remove_file(self, path: str) -> None:
        ...
        try:
            self._sftp_connection.remove(path)
        except Exception as e:
    ...
    def is_file(self, path: str) -> bool:
        ...
        """
        return self._sftp_connection.isfile(path)
    ...
    def is_dir(self, path: str) -> bool:
        ...
        """
        return self._sftp_connection.isdir(path)
    ...
    def path_exists(self, path: str) -> bool:
        ...
        """
        return self._sftp_connection.lexists(path)
    ...
    def make_dirs(self, path: str) -> None:
        ...
        if not self.path_exists(path):
            try:
                self._sftp_connection.mkdir(path)
            except Exception as e:
    ...
    def set_socket_timeout(self, socket_timeout_sec):
        ...
        """
        self._sftp_connection.timeout = socket_timeout_sec
    ...
    def _download_file(self, destination: str, delete_origin: bool, source: str) -> None:
        ...
        try:
            self._sftp_connection.get(source, destination_path)
        except Exception as e:
    ...
    def _put_file(self, source: str, destination: str) -> None:
        return self._sftp_connection.put(source, destination)
...
class SftpClientBuilder:
    ...
    def build_sftp_client(
            ...
            retry_interval_sec: int = RETRY_STEP_SEC,
            retry_step_sec: int = RETRY_STEP_SEC
    ):
        ...
        prefix: str = cls._get_prefix(host, port, username, password)
        sftp: 'pysftp.Connection' = cls._connect(host, port, username, password, private_key, private_key_pass,
                                                 connection_tries, retry_interval_sec, retry_step_sec)
        handler_builder: HandlerBuilder = HandlerBuilder(sftp, prefix, private_key, private_key_pass)

        return SftpClient(sftp, handler_builder, prefix)
    ...
    def _connect(
        ...
        while not sftp:
            try:
                ...
                logger.verbose('Connected to {}', host)  # type: ignore
            except Exception as e:
                tries -= 1
                if not tries:
                    raise e
