...
class FilterSeries(FilterSeriesBase):
    ...
    def process_series(self, task, series_entries, config):
        ...
        for entity, entries in sorted(
            ...
            entries.sort(
                key=lambda e: (
                    e['quality'],
                    e['series_parser'].episodes,
                    e['series_parser'].proper_count,
                ),
                reverse=True,
            )
    ...
    def process_propers(self, config, episode, entries):
        ...
        for entry in best_propers:
            if (
                entry['quality'] in downloaded_qualities
                and entry['series_parser'].proper_count > downloaded_qualities[entry['quality']]
            ):
                entry.accept('proper')
