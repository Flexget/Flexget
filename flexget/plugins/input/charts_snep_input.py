...
source_enum = ['all_album', 'disk_album', 'digit_album',
               'digit_single', 'stream_single',
               'compil', 'back_catalog', 'radio']


def valid_config(config):
    return config not in source_enum


# TODO: Use db for improved perfs?
class ChartsSnepInput(object):
    """
    Create an entrie for each charted single in your charts request.

    Syntax:
    charts_snep_input: <one of : ['all_album', 'disk_album', 'digit_album',
               'digit_single', 'stream_single',
               'compil', 'back_catalog', 'radio']>
    """
    schema = {
    ...
    def on_task_input(self, task, config):
        ...
        if not valid_config(config):
            log.warning("'%s' is an unknow charts type for charts_snep. Please to use one of them : %s."
                        % (config, ', '.join(source_enum)))
            return []
