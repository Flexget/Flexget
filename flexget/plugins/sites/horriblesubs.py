...
class HorribleSubs(object):
    ...
    def horrible_get_downloads(requests, title, page_url):
        ...
        try:
            ...
            return entries
        for span in soup.findAll('span', attrs={'class': 'dl-type'}):
            # skip non torrent based links
            if 'hs-ddl-link' in span.attrs['class'] or 'hs-xdcc-link' in span.attrs['class']:
                continue
            ttitle = '{0} [{1}]'.format(title,re.sub(r'.*-', '', span.parent['id']))
    ...
    def on_task_input(self, task, config):
        ...
        return HorribleSubs.horrible_entries(scraper, 'https://horriblesubs.info/api.php?method=getlatest')
    ...
    def search(self, task, entry, config):
        ...
        for search_string in entry.get('search_strings', [entry['title']]):
            ...
            results = HorribleSubs.horrible_entries(
                scraper, 'https://horriblesubs.info/api.php?method=search&value={0}'.format(search_string))
