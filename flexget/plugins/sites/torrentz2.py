...
log = logging.getLogger('torrentz2')
...
class Torrentz2(object):
    """Torrentz2 search and urlrewriter"""
    ...
    def url_rewrite(self, task, entry):
        """URL rewrite torrentz2 domain url with infohash to any torrent cache"""
        ...
        for search_string in entry.get('search_strings', [entry['title']]):
            ...
            for domain in ['eu', 'me', 'ch', 'in']:
                ...
                try:
                    ...
                    log.warning('torrentz2.%s connection failed. Error: %s' % (domain, err))
                    continue
                except requests.RequestException as err:
                    raise plugin.PluginError('Error getting torrentz2 search results: %s' % err)

            else:
                raise plugin.PluginError('Error getting torrentz2 search results')

            if not r.content.strip():
                raise plugin.PluginError('No data from %s. Maybe torrentz2 is blocking the FlexGet User-Agent' % url)
