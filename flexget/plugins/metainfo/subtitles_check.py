...
class MetainfoSubs(object):
    ...
    def lazy_subs_check(self, entry, field):
        if not ('location' in entry) or ('$RECYCLE.BIN' in entry['location']) or \
            not os.path.exists(entry['location']) or entry.get('subtitles', eval_lazy=False):
            return
        import subliminal
        try:
    ...
    def on_task_metainfo(self, task, config):
        ...
        for entry in task.entries:
            entry.register_lazy_fields(['subtitles'], self.lazy_subs_check)
