    ...
    def lookup(self, entry, nfo_filename):
        ...
        if nfo_filename is None:
            ...
            if 'nfo_id' in fields:
                ...
                # We will perform an IMDB search later the fields add here might be
                # overwritten
                self.add_imdb_fields_to_dict(fields)

        # Let's search metadata in IMDB. If we found an nfo file containing an
        # ID we have already added the 'imdb_id' field and this will make the
        # IMDB lookup below find the correct movie no matter what the title and
        # filename are.
        imdb_lookup = ImdbLookup()
        imdb_lookup.lookup(entry)  # This will update the entry

        # Add corresponding imdb fields to the dictionary We might have found
        # more imdb metadata in the nfo file, such as the title, the plot,
        # etc. We will replace these fields from the previous IMDB lookup with
