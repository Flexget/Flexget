...
class OutputPushbullet(object):
    ...
    def on_task_output(self, task, config):
        ...
        destinations = []
        for typ, val in [('device_iden', devices), ('email', emails)]:
            destinations.extend([(x, typ) for x in val])
        ...
        for entry in task.accepted:
            ...
            for apikey in apikeys:
                if len(destinations):
                    for destination, destination_type in destinations:
                        self.send_push(task, apikey, title, body, url, destination, destination_type)
                else:
