...
class Entry(LazyDict):
    ...
    def reject(self, reason=None, **kwargs):
        ...
        if self.get('immortal'):
            ...
            log.info('Tried to reject immortal {} {}', self['title'], reason_str)
    ...
    def fail(self, reason=None, **kwargs):
        log.debug("Marking entry '{}' as failed", self['title'])
        if not self.failed:
            self._state = 'failed'
            self.trace(reason, operation='fail')
            log.error('Failed {} ({})', self['title'], reason)
    ...
    def __setitem__(self, key, value):
        ...
        try:
            log.trace('ENTRY SET: {} = {!r}', key, value)
        except Exception as e:
            log.debug('trying to debug key `{}` value threw exception: {}', key, e)
    ...
    def take_snapshot(self, name):
        ...
        if snapshot:
            if name in self.snapshots:
                log.warning('Snapshot `{}` is being overwritten for `{}`', name, self['title'])
