...
class TestNextSeriesEpisodes:
    _config = """
        ...
        tasks:
          ...
          test_next_series_episodes_no_backfill:
            ...
            - Test Series 1
            max_reruns: 0
          test_next_series_episodes_backfill_cap:
            next_series_episodes:
              backfill: yes
              backfill_cap: 10
            series:
    ...
    def test_next_series_episodes_backfill_cap(self, execute_task):
        self.inject_series(execute_task, 'Test Series 1 S01E01')
        self.inject_series(execute_task, 'Test Series 1 S01E13')
        task = execute_task('test_next_series_episodes_backfill_cap')
        assert len(task.all_entries) == 1, "missing episodes over cap. Should not backfill"
        self.inject_series(execute_task, 'Test Series 1 S01E12')
        task = execute_task('test_next_series_episodes_backfill_cap')
        assert len(task.all_entries) == 11, "missing episodes less than cap. Should backfill"
