...
class TestRadarrListActions(object):
    config = """
        ...
        tasks:
            ...
            list_clear:
              what:
                - radarr_list:
                    ...
                    api_key: a76ab21e674a4c88a4d757f21eb91edd
                    port: 7878
            mock:
          ...
          match_radarr_list:
            mock:
              - { title: 'Despicable.Me.2.2013.1080p.BluRay.x264-VeDeTT', imdb_id: 'tt1690953', tmdb_id: 93456 }
              - { title: 'Sinister.2.2015.720p.BluRay.x264-BLOW', imdb_id: 'tt2752772', tmdb_id: 283445 }
              - { title: 'Crimson.Peak.2015.720p.BluRay.x264-SPARKS', imdb_id: 'tt2554274', tmdb_id: 201085 }
              - { title: 'Deadpool.2016.1080p.BluRay.x264-SPARKS', imdb_id: 'tt1431045', tmdb_id: 293660 }
              - { title: 'Kung.Fu.Panda.3.2016.720p.BluRay.x264-SPARKS', imdb_id: 'tt2267968', tmdb_id: 140300 }
            list_match:
              from:
                - radarr_list:
                    base_url: http://192.168.0.60
    ...
    def test_radarr_list_actions(self, execute_task):
        ...
        assert task.find_entry('accepted', title='Despicable.Me.2.2013.1080p.BluRay.x264-VeDeTT'), \
          "movie should have been matched but it wasn't"
        assert task.find_entry('accepted', title='Crimson.Peak.2015.720p.BluRay.x264-SPARKS'), \
          "movie should have been matched but it wasn't"
        assert task.find_entry('accepted', title='Deadpool.2016.1080p.BluRay.x264-SPARKS'), \
          "movie should have been matched but it wasn't"
        assert task.find_entry('undecided', title='Sinister.2.2015.720p.BluRay.x264-BLOW'), \
          "movie should not have been matched but it was"
        assert task.find_entry('undecided', title='Kung.Fu.Panda.3.2016.720p.BluRay.x264-SPARKS'), \
          "movie should not have been matched but it was"
