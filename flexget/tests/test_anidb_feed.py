...
class TestAnidbFeed(object):
    ...
    def test_anidb_feed(self, execute_task):
        ...
        for entry in task.entries:
            ...
            assert isinstance(entry['content_size'], int), 'expecting int size type for content_size'
            assert entry['anidb_name'], 'expecting anidb_name'
            assert entry['anidb_fid'], 'expecting anidb_fid'
            assert isinstance(entry['content_size'], int), 'expecting int size type for anidb_fid'
            assert entry['anidb_feed_size'], 'expecting anidb_feed_size'
            assert isinstance(entry['anidb_feed_size'], int), 'expecting int size type for anidb_feed_size'
