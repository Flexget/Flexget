from flexget.plugins.filter.movie_queue import queue_add, queue_get, QueuedMovie
...
class TestMovieQueue(FlexGetBase):
    ...
    def test_movie_queue_accept(self):
        queue_add(title='MovieInQueue', imdb_id='tt1931533', tmdb_id=603)
    ...
    def test_movie_queue_add(self):
        ...
        assert entry.get('imdb_id', eval_lazy=False) == 'tt1931533'
        assert entry.get('tmdb_id', eval_lazy=False) == 603
        assert entry.get('quality', eval_lazy=False) == 'any'
    ...
    def test_movie_queue_remove(self):
        queue_add(title='MovieInQueue', imdb_id='tt1931533', tmdb_id=603)
        queue_add(title='KeepMe', imdb_id='tt1933533', tmdb_id=604)
