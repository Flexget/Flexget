...
class TestUoccinReader(FlexGetBase):
...
1431198108565|series|272135.2.5|subtitles|eng,ita
    ...
    def test_read(self):
        self.execute_task('test_sync')
        self.execute_task('test_sync_chk')
        entry = self.task.find_entry(imdb_id='tt0047034')
        ...
        entry = self.task.find_entry(imdb_id='tt0057012')
        ...
        entry = self.task.find_entry(tvdb_id='80379', series_season=8, series_episode=24)
        assert entry.get('uoccin_watchlist', False), \
        ...
        entry = self.task.find_entry(tvdb_id='272135', series_season=2, series_episode=5)
        assert entry.get('uoccin_collected', False), \
...
class TestUoccinWriters(FlexGetBase):
    ...
    def test_write(self):
        self.execute_task('test_del')
        self.execute_task('test_chk')
        entry = self.task.find_entry(imdb_id='tt0047034')
