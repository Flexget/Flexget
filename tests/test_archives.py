...
class TestArchiveFilter(object):
    ...
    def test_rar(self, execute_task):
        ...
        assert task.find_entry('accepted', title='test_rar'), 'test_rar.rar should have been accepted'

    def test_zip(self, execute_task):
        """Test Zip acceptance"""
        task = execute_task('test_archives')
        assert task.find_entry('accepted', title='test_zip'), 'test_zip.zip should have been accepted'

    def test_invalid(self, execute_task):
        """Test non-archive rejection"""
        task = execute_task('test_archives')
        assert task.find_entry('rejected', title='invalid'), 'invalid.zip should have been rejected'
